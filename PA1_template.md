...
Created by English Gardener on 10 June 2015
...
```r
#Uncomment the following line to unzip the input data first, if compressed as .zip file. 
#unzip("activity.zip") 
...
My chosen strategy is to fill each NA value with the mean for that 5-minute interval in the steps column.
...
## [1] 10766.19
```
Compare the new mean and median values with the two before imputing missing data:
...
## [1] 1.188679
```
In conclusion, after imputing the missing data, the new mean of total steps taken per day is the same as that of the old mean; the new median of total steps taken per day is greater than that of the old median.
